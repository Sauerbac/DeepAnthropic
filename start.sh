...
# Start router in its own dir (it reads ./config.toml); exec so $! is the exe itself.
...
echo "Router up on :$port. Launching Claude Code (default tier = deepseek-v4-pro; /model opus for real Opus)."
claude "$@"
